...
# 「첨부」도 별표와 같은 자리에 온다(금투협 2건). 빠뜨리면 점프 목록에 안 나온다.
# 은행연은 원문을 그대로 쓴 통짜 본문이라 to_text() 형식이 아니라 "<별표1>"(홑화살괄호,
# 공백·자릿수 채움 없음)로 온다 — 대괄호·4자리만 허용하면 이 소스는 별표가 전부 빠진다.
_TABLE_HEAD = re.compile(r"^[\[<](?:별표|별지|서식|별책|첨부)(?:\s*\d{1,4}(?:의\d+)?)?[\]>]")
...
def article_index(text):
    ...
    for lineno, line in enumerate(lines):
        ...
        if stripped.startswith(("(", "（")):
            continue
        # 은행연처럼 "별표 / 서식" 구분선이 아예 없는 소스는 위 명시적 트리거로는
        # in_tables 가 영영 안 켜져 별표가 통째로 안 보인다(실측: 은행 광고심의 기준
        # 및 세칙 — 점프 목록 별표 0개). 첫 별표류 헤더를 만나는 순간을 진입점으로
        # 삼는다. 부칙 전 구간(조문 본문)에는 이런 헤더가 없는 걸 확인했으므로
        # (43개 대상 전수 재검증 시 새 소스가 추가되면 다시 확인 필요) 안전하다.
        if not in_tables and _TABLE_HEAD.match(stripped):
            in_tables = True
            in_addenda = False   # 안 꺼두면 아래 elif in_addenda 에서 그대로 걸러진다
        ...
        extra = {}
        if in_tables:
            fname_title, srcfile = _table_meta(stripped)
            body = _title_from_body(lines, lineno)
            # 본문에서 뽑은 제목이 있으면 그것만 쓴다 — 파일명에서 뽑은 「별지 제50호」는
            # 키(레이블)에 이미 있는 번호를 문자로 반복할 뿐이라 붙이면 "[별지 0050]
            # 별지 제50호 펀드…" 처럼 중복·장황해진다. 본문 추출이 실패했을 때만
            # 파일명 기반 제목으로 폴백한다.
            title = body or fname_title
...
# 은행연 소스는 홑화살괄호("<별표1>")를 쓰므로 대괄호와 함께 인식해야 한다.
_L_TABLE = re.compile(r"^[\[<](별표|별지|서식|별책|첨부)(?:\s*(\d{1,4})(?:의(\d+))?)?[\]>]$")
