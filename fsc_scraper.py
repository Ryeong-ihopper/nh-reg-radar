...
TARGETS = {
    ...
    # 예금성 광고의 기본금리 표시 기준. 보도자료 본문뿐이라 개정되지는 않지만,
    # 내용이 심의 근거로 쓰이므로 파이프라인에 넣어 뷰어·검색에 노출한다.
    "특판 예적금 광고 준수 필요사항": {
        "검색어": "특판 예적금 광고",
        "파일조건": ["특판 예적금"],
        "확장자": ".pdf",
...
# 여기에 넣는 기준: **내용이 심의 근거로 쓰이는가.** 개정 여부가 아니다.
# 안 바뀌는 문서라도 수집해 두면 뷰어에 뜨고 검색되고 DB 에 들어간다.
# 개정이 없으면 매달 '동일' 로 지나갈 뿐이라 비용은 요청 한 번이다.
# (처음에는 '별첨 기준문서가 있는가'로 갈랐는데, 그러면 안 바뀌지만 실무에 필요한
#  자료가 아무도 안 보는 폴더로 밀려나서 기준을 바꿨다.)
