...
**공식 명칭이 어색해 보여도 그대로 둔다** — 이 이름이 법제처 검색 키이고, 심의 근거로
인용될 이름이다. 실제로 오타처럼 보이지만 정상인 사례:

| 공식 명칭 | 오타로 보이는 지점 |
|---|---|
| 표시**ㆍ**광고의 공정화에 관한 법률 | 구분자가 `·`(가운뎃점)이 아니라 `ㆍ`(아래아) |
| 금융소비자**보호**에 관한 감독규정 시행세칙 | 상위 「금융소비자 보호에 관한 감독규정」은 띄어 쓰는데 시행세칙만 붙어 있다. 법제처 등록명이 그렇다 |
