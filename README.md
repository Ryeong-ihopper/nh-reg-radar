...
| 출처 | 건수 | 방식 | 변경 감지 키 |
|---|---:|---|---|
| 국가법령정보센터 — 법령 | 15 | Open API | `일련번호\|시행일자\|공포번호` |
| 국가법령정보센터 — 행정규칙 | 10 | Open API (필드명 상이) | 〃 |
| 금융투자협회 | 4 | 웹 엔드포인트 + HWP 첨부 | `seq\|historySeq` |
| 여신금융협회 | 2 | HWP 첨부 (세션·토큰) | 파일 sha256 |
| 전국은행연합회 | 1 | HWP 첨부 (세션·토큰) | 파일 sha256 |
| 금융위원회 | 3 | 게시판 첨부 (제목 검색) | 첨부 sha256 |

규모: 조문 3,784 · 부칙 941 · 별표 438 · 첨부 762개(74MB)
...
python check_updates.py             # 첫 수집 (20~40분)
```
...
| 명령 | 동작 |
|---|---|
| `check_updates.py` | 감지 → 변경분 재수집 → DB 적재 → 품질 점검 |
| `check_updates.py --dry` | 감지만 |
| `check_updates.py --deep` | 본문 해시까지 비교 (버전 동일, 내용만 수정된 경우) |
| `check_updates.py --only "여신"` | 이름 부분일치 대상만 (실패분 재시도) |
| `discover_targets.py` | 대상 추가 전 공식 명칭 조회 |
| `quality_check.py` | 수집 품질 점검 |
| `build_review.py` | 원본 대조 화면 → `output/_review/review.html` |
| `validate_outputs.py --live` | 수집 결과 ↔ 공식 원문 대조 |
| `uvicorn api:app` | 조회 API → `/docs` |

대상 추가는 `targets.json`. 명칭은 `discover_targets.py`로 조회한 값을 쓸 것
...
진입점: `check_updates.py` → `law_scraper.py` → `diff_report.py`
...
**표 구조 보존, 폴백 없음.** 첨부 757개 중 64%가 표 포함(2,439개), 별표·별지는 표가 내용이다.
...
`build_review.py` → `output/_review/review.html` (단일 파일, 오프라인)
...
- API 인증 없음 — 로컬 전용
- SQLite 단일 노드 — 다중 접속 시 `sql/regulation_schema.sql`(PostgreSQL)로 이전
